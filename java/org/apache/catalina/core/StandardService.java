...
public class StandardService extends LifecycleMBeanBase implements Service {
    ...
    protected void startInternal() throws LifecycleException {
        ...
        if (engine != null) {
            synchronized (engine) {
                engine.start();                         //启动源码39：调用engine.start()
            }
        }

        synchronized (executors) {
            for (Executor executor: executors) {
                executor.start();                       //启动源码40：调用executor.start()
            }
        }
            ...
            for (Connector connector: connectors) {
                try {
                    ...
                    if (connector.getState() != LifecycleState.FAILED) {
                        connector.start();              //启动源码41：connector.start()
                    }
                } catch (Exception e) {
                    log.error(sm.getString(
                            "standardService.connector.startFailed",
    ...
    protected void initInternal() throws LifecycleException {
        ...
        for (Executor executor : findExecutors()) {
            ...
            executor.init();                                    //启动源码21：初始化executor连接池
        }

        // Initialize mapper listener
        mapperListener.init();                                  //启动源码22：初始化mapperListener

        // Initialize our defined Connectors
        synchronized (connectorsLock) {
            for (Connector connector : connectors) {
                try {
                    connector.init();                           //启动源码23：初始化connector
                } catch (Exception e) {
