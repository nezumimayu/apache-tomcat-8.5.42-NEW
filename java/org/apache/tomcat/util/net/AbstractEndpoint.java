...
public abstract class AbstractEndpoint<S> {
    ...
    public abstract void bind() throws Exception;               //启动源码31：Tomcat8.5的IO模型默认采用NIO
    public abstract void unbind() throws Exception;
    public abstract void startInternal() throws Exception;      //启动源码46：Tomcat8.5的IO模型默认采用NIO
    ...
    public final void start() throws Exception {
        ...
        startInternal();                                    //启动源码45：调用startInternal()
    }

    protected final void startAcceptorThreads() {
        int count = getAcceptorThreadCount();
        acceptors = new Acceptor[count];                    //启动源码48：初始化Acceptor数组（Acceptor是接收客户端请求的对象）

        for (int i = 0; i < count; i++) {
            acceptors[i] = createAcceptor();                //启动源码49：通过createAcceptor()为数组赋值
     ...
     * @return the acceptor
     */
    protected abstract Acceptor createAcceptor();                                //启动源码50：Tomcat8.5的IO模型采用NIO
